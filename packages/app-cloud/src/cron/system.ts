...
/**
 * `opts.runner` is a service exposing `{ trigger }`; the returned system
 * provisions it alongside the scheduler that fires `opts.schedule` at it. The
 * system's boundary deps mirror the runner's own deps, so the parent wires the
 * real work target through them, e.g.
 * `provision(cron({ schedule, runner }), { worker: worker.rpc })`. `opts.name`
 * sets the system name (default `'cron'`). Exposes nothing.
 */
