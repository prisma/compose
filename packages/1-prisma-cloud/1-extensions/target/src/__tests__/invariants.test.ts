...
describe('invariant 4: environment touches are confined to the config serializer and the control factory', () => {
  test("the process-env token appears only in serializer.ts (param read+stash, reserved-provider-param read+stash, secret double-lookup+stash, env-sourced param double-lookup), control/extension.ts's prismaCloud(), preflight.ts (shell token + fill-missing lookup), teardown.ts (shell token), and compute.ts (exposes the resolved port as PORT) (the extension factory's env read, ADR-0017 — PRISMA_WORKSPACE_ID + optional PRISMA_REGION; ADR-0019 — PRISMA_PROJECT_ID + optional PRISMA_BRANCH_ID)", () => {
    ...
    expect(hits.sort((a, b) => a.file.localeCompare(b.file))).toEqual([
      ...
      { file: 'control/extension.ts', count: 4 },
