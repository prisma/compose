...
describe('entry map: core splits into authoring + deploy + pure utils — no runtime entry', () => {
  test("package.json exports '.', './deploy', and the ./casts + ./assertions utilities", () => {
    ...
    expect(codeEntries.sort()).toEqual(['.', './assertions', './casts', './deploy']);
  });
});
...
describe("invariant 2: the '.' authoring entry bundles lean", () => {
  ...
  test('the ecosystem-seam adapters (@prisma/app-node, @prisma/app-nextjs) are equally lean', async () => {
    const out = await Bun.build({
      entrypoints: [
        path.join(pkgDir, '..', 'app-node', 'src', 'index.ts'),
        path.join(pkgDir, '..', 'app-nextjs', 'src', 'index.ts'),
      ],
      target: 'bun',
    });
    expect(out.success).toBe(true);
