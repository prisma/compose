...
// hex-composition.test.ts's runtime proof that a real hex-provisioned
...
// hex-composition.test.ts's "untyped inputs" runtime test).
const untypedEnd = () =>
  dependency({
    type: 'fake/http',
    connection: conn({ url: { type: 'string' } }, (v) => ({ url: v.url })),
...
hex('untyped-forward', { deps: { anything: untypedEnd() } }, ({ inputs, provision }) => {
...
// depth 1: the same pass-through pattern, wrapping `innerHex`.
const midHex = hex(
  'mid',
  { deps: { verify: verifyEnd() }, expose: { verify: verifyContract } },
  ({ inputs, provision }) => {
    const inner = provision('inner', innerHex, { verify: inputs.verify });
    return { verify: inner.verify };
  },
);

// root: wires a real producer's port down through 2 boundaries of `midHex`,
...
hex('root-ok', {}, ({ provision }) => {
  const p = provision('provider', verifyProvider());
  const mid = provision('mid', midHex, { verify: p.verify });
...
hex('root-bad', {}, ({ provision }) => {
  const p = provision('providerX', verifyProvider());
  const mid = provision('midX', midHex, { verify: p.verify });
