...
/**
 * The bundler firewall (H2, node-owned loads): `nextjs()`'s `assembler` field
 ...
 */
describe("nextjs()'s factory carries no static import edge to /assemble", () => {
  test('index.ts never calls import()/require() with a literal /assemble specifier', () => {
    ...
    expect(source).toContain("assembler: '@prisma/app-nextjs/assemble'");
