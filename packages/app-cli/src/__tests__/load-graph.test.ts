...
import { Load, LoadError } from '@prisma/app';
...
describe('the pipeline surfaces core Load() errors as-is (deploy-cli.md step 2)', () => {
  test('a service with an unwired dependency input fails naming the input and the composing system', async () => {
    ...
    expect(() => Load(entry.root)).toThrow(/deploy the system instead/);
  });

  test('a system root loads its provisioned services under their provision ids', async () => {
    const entry = await loadEntry(fixture('valid-system.ts'), import.meta.dir);

    const graph = Load(entry.root);

    expect(graph.root.node.kind).toBe('system');
