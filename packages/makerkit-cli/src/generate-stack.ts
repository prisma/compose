 ...
 * module at `.makerkit/alchemy.run.ts` inside the process's own working
 ...
 * independently runnable (`alchemy deploy .makerkit/alchemy.run.ts`) to
 * bisect a CLI bug from an Alchemy bug.
 */
import * as fs from 'node:fs';
import * as path from 'node:path';
import type { AssembledServices } from '@makerkit/assemble';
...
export function renderStackFile(input: StackFileInput): string {
  ...
  return `// Generated by \`makerkit deploy\`/\`makerkit destroy\` — overwritten on every
