...
describe('importFromEntry() — entry-anchored resolution', () => {
  ...
  test('a missing pack throws an AssembleError naming the pack, the entry dir, and the fix', async () => {
    ...
    await expect(importFromEntry(entryPath, '@prisma/does-not-exist', 'target')).rejects.toThrow(
      AssembleError,
    );
    await expect(importFromEntry(entryPath, '@prisma/does-not-exist', 'target')).rejects.toThrow(
      new RegExp(
        `Cannot resolve "@prisma/does-not-exist/target" from ${entryDir.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')} — the app's package \\(the one containing the entry module\\) must depend on "@prisma/does-not-exist"`,
      ),
    );
  });
