 ...
 * otherwise match. `prisma-composer-state` is the current, in-use hosted
 * deploy-state control plane (packages/alchemy/src/state/bootstrap.ts).
 */
export const PROTECTED_PROJECT_NAMES: readonly string[] = ['prisma-composer-state'];
 ...
 * rename to `prisma-composer-state`. None of these names may ever collide with
