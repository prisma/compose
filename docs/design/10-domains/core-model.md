...
| `@prisma/composer/arktype` | `secretString()` — the arktype spelling of a `SecretString` input leaf (ADR-0041). Opt-in: no other entry imports it, so a Zod app never loads arktype | `arktype` |
