...
> Naming: the grouping unit is called a **Hex (Subsystem)** — "Hex" is the
> working name; "Subsystem" is the fallback if the team prefers a literal term.
> "Hex" thereafter.

## The three planes

- **Authoring plane (MakerKit)** — what the developer writes. Nouns: **Hex**,
...
1. **A Hex is an authoring/reasoning unit, not a hosting unit.** It lowers to a
   *subgraph* of hosting primitives — its Services become Compute services, its
   ...
   The hosting plane can still co-locate (several Hexes sharing one Environment
   Database, carved by the aggregate contract). Boundaries and isolation live at
   the *authoring* plane, not the hosting plane.

2. **MakerKit owns the lowering and the emit step.** Alchemy gives the definition
   language and (optionally) provisioning; it has no "serialize the desired graph
   as a portable artifact" step. MakerKit adds that — the topology artifact an
   external orchestrator consumes.

3. **Definition language *and* engine.** MakerKit adopts Alchemy/Effect for the
...
- **Compute** — Bun on Unikraft microVMs, HTTP-first, scale-to-zero. Deploy
  ...
  A MakerKit Service lowers almost directly to this.
...
- Wiring today is **env vars baked at deploy** (`DATABASE_URL`). MakerKit's
  no-globals DI is a layer on top: it reads those at the boundary and injects
  typed handles, so user code never reads the environment.

Notes: there is no general Prisma provisioning orchestrator (Foundry deploys
Compute only) — MakerKit provisions via Alchemy's engine instead (see below).
Prisma offers no first-party object/file storage; a Hex that needs storage uses a
**BYO Alchemy resource** (R2, S3, …). Accelerate and Pulse are discontinued.

## Resources: first-class vs BYO

A MakerKit Resource is an Alchemy resource surfaced as a typed **capability**. A
Hex's Input requires a capability; a Resource's Output provides one (via an Alchemy
Layer); the wire is valid iff the provided capability satisfies the required one —
Alchemy type-checks it.

- **First-class** (Postgres): MakerKit-native treatment — Postgres data uses
...
- **BYO** (object storage, cache, queues, third-party): *any* Alchemy resource,
  ...
  vendor — swap R2 for S3 by swapping the Layer; the Hex is unchanged. This is how
  a Hex uses, e.g., file storage today.
