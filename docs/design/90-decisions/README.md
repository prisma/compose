...
- [ADR-0018](ADR-0018-config-params-carry-a-caller-owned-schema.md) — Config params carry a caller-owned Standard Schema, not a fixed `ParamType` enum; any shape becomes expressible with no core change.
- [ADR-0019](ADR-0019-the-target-owns-config-serialization.md) — The target owns config serialization; params serialize to key/value string pairs, and the param that feeds a target's Service node is the target's type, carrying its serializer.
- [ADR-0020](ADR-0020-scheduled-work-is-a-driver-not-a-resource.md) — Scheduled work (cron) is a driver System — a service that depends on what it calls — with the schedule as build-time config, not a loadable resource.
