...
A system declares the same boundary a service does: a `Deps` map of typed
...
`provision()` accepts a system wherever it accepts a service, handing back the
same `ProvisionedRef` of exposed ports — so systems nest, and a consumer cannot
tell a provisioned system from a provisioned service. The two differ only in
opacity: a service's body is a black box; a system's body is topology the
framework can see.
...
schema looks like. That is a reusable system: not a SaaS you call, not a library
you embed in your process — a deployable component with a typed boundary.

For that to work a system needs a boundary, and the model already owns the right
...
that boundary for systems is the whole decision. It keeps the port mechanic
...
The boundary is also what makes a system *fakeable*. A consumer depends on a
contract, never on the system: the storefront above requires something
...
Distribution then falls out of decisions already made. A system package ships
...
`node_modules/@acme/auth-system/` exactly as it would in the app's own tree, and
...
(ADR-0008). Version compatibility is the package manager's job: a system
...
One requirement lands on the deploy tooling: a system's internal choice of build
adapter must stay internal — a consuming app must not have to declare
`@prisma/app-nextjs` because some installed system happens to use Next inside.
...
- The authoring surface is `system(name, { deps?, expose? }, body)`: the body
  takes its inputs and `provision`, and returns a port for each declared output.
  An empty boundary (`system(name, {}, body)`) is the closed, deploy-root form —
...
- The serialized-config/stash encoding is versioned public surface: a system
  published against a pack freezes reader code at publish time, so a format
  change is a semver-major of the pack.
- Reusable systems are pack-specific: a system authored in `@prisma/app-cloud`'s
...
- **Systems stay root-only wiring scripts** (a system with no boundary) —
  forecloses reuse: a system that cannot be wired cannot be installed, faked, or
