...
An application root carries a `prisma-app.config.ts`. The config statically
...
a node's `extension` field (`"@prisma/app-cloud"`) and its `type`
...
- **A loader thunk with a literal import on the node**
  (`loadAssembler: () => import("@prisma/app-node/assemble")`) — the literal
